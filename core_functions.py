def TPMNorm(fragLength, dataFrame, lengthDict):
    ...
    scaleDataframe = pd.DataFrame(data = scaleMatrix.T, 
                                  columns=count.columns, index=count.index)
    count = count.divide(scaleDataframe)
    count = count + 1
    count = np.log2(count)
    return count

def pcaPlotSea(comp, dataFrame, filename, x_inv = False, y_inv = False, legend = False):
    '''
    ...
    df_pca = pd.DataFrame(x.transform(count_t), 
                          columns=columns, index=count_t.index)
    df_pca['Day'] = df_pca.index.astype('category')
    size = pca.explained_variance_ratio_
    plt.figure()
    f, ax = plt.subplots(figsize=(12, 12))
    ax = sns.scatterplot(x="pca_0", y="pca_1",
                     hue="Day", sizes=(10, 200),
                     data=df_pca, legend=legend, palette=palette, s=400, linewidth=0)
    sns.despine(f, right=True, top=True, offset=10, trim=True)
    ax.set(xlabel= "{:.1f}".format(size[0]*100) + 
           "% variance", ylabel= "{:.1f}".format(size[1]*100) + "% variance")
    ax.tick_params(direction='out', length=10, width=5)
    my_xticks = ax.get_xticks()
    plt.xticks([my_xticks[0], 0.0, my_xticks[-1]], 
               visible=True, rotation="horizontal")
    my_yticks = ax.get_yticks()
    plt.yticks([my_yticks[0], 0.0, my_yticks[-1]], 
               visible=True, rotation="horizontal")
...
def plotClusterMap(dataFrame, filename, supressRowDend=False, supressColDend=False, clusterColumn=True, clusterRow=True, vUpper=10.0, colorColumn=0):
    '''
    ...
    def aggloCluster(numberColumn, matrix):
        ...
        #stages
        model = AgglomerativeClustering(n_clusters=numberColumn, linkage="average")
        model.fit(matrix)
        labels = (model.labels_).tolist()
        return labels
    
    def SaveFigure(ClusterMap, supressRowDend=supressRowDend, supressColDend=supressColDend, filename=filename, vUpper=vUpper):
    ...
    if(colorColumn > 0):
        ...
        labels = [i for x,y in zip(sns.color_palette("Set2", len(set(labels))),
        ...
        ClusterMap = sns.clustermap(dataFrame, cmap="inferno", linewidths=0, 
                            figsize=(6, rowLen/8), robust=True, col_cluster=clusterColumn, 
                            row_cluster=True, vmin=np.log2(1), vmax=vUpper, col_colors=Column_colors, yticklabels=False)
        ...
        ClusterMap = sns.clustermap(dataFrame, cmap="inferno", linewidths=0, 
                            figsize=(6, rowLen/8), robust=True, col_cluster=clusterColumn, 
                            row_cluster=True, vmin=np.log2(1), vmax=vUpper, yticklabels=False)
        ClusterMap.cax.set_visible(False)
        ax = ClusterMap.ax_heatmap
        ax.set_ylabel("")
        SaveFigure(ClusterMap)
        
def plotClusterMapBi(dataFrame, filename, supressRowDend=False, supressColDend=False, vUpper=10.0):
    '''
    ...
    if(colorRow > 0):
        ...
        labels = [i for x,y in zip(sns.color_palette("tab20b", len(set(labels))),
                                      sorted(set(labels),key=labels.index)) 
                     for i in np.repeat([x],labels.count(y),axis=0).tolist()]
        group_lut = dict(zip(map(int, rowNames), labels))
        Row_colors = pd.Series(group_lut)
        ylabels = [x for x in itertools.chain.from_iterable(itertools.zip_longest(['—']*int(rowLen/2),['+']*int(rowLen/2))) if x]
        ClusterMap = sns.clustermap(dataFrame, cmap="inferno", linewidths=0, 
                            figsize=(6, rowLen/8), robust=True, col_cluster=False, 
                            row_cluster=False, vmin=np.log2(1), vmax=vUpper, row_colors=Row_colors, yticklabels=ylabels)
        ClusterMap.cax.set_visible(False)
        ax = ClusterMap.ax_heatmap
        ax.set_ylabel("")
        ...
        ClusterMap = sns.clustermap(dataFrame, cmap="inferno", linewidths=0, 
                            figsize=(6, rowLen/8), robust=True, col_cluster=False, 
                            row_cluster=False, vmin=np.log2(1), vmax=vUpper, yticklabels=False)
        ClusterMap.cax.set_visible(False)
        ax = ClusterMap.ax_heatmap
        ax.set_ylabel("")
        SaveFigure(ClusterMap)
...
def plotClusterMapUnscaled(dataFrame, filename, supressRowDend=False, supressColDend=False, clusterColumn=True, clusterRow=True, vUpper=10.0, colorColumn=0, l=10, w=10):
    '''
    Function to create clustermap based on seaborn clustermap
    function. It takes dataframe containing counts with index 
    as gene names and filename for the output. It also allows
    ...
    if(colorColumn > 0):
        ...
        ClusterMap = sns.clustermap(dataFrame, cmap="inferno", linewidths=0, 
                            figsize=(l, w), robust=True, col_cluster=clusterColumn, 
                            row_cluster=True, vmin=np.log2(1), vmax=vUpper, col_colors=Column_colors, yticklabels=False)
        ClusterMap.cax.set_visible(False)
        ax = ClusterMap.ax_heatmap
        ax.set_ylabel("")
